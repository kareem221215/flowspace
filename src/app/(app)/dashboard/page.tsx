...
export default function DashboardPage() {
  const { currentUser, todos, notes, projects, files, notifications, messages, channels } =
    useAppStore()
                      ...
                      <div className="flex items-center gap-3 px-5 py-3.5 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors group">
                        <div
                          className={`w-2 h-2 rounded-full shrink-0 ${
                            todo.status === 'in_progress' ? 'bg-amber-400' : 'bg-slate-200'
                          }`}
                        />
                        <div className="flex-1 min-w-0">
                          <p className="text-sm text-slate-800 truncate group-hover:text-slate-900">
                            {todo.title}
                          </p>
                          {todo.due_date && (
                        ...
                        <span className={`badge ${p.bg} ${p.color} shrink-0`}>
                          {p.label}
                        </span>
                      </div>
                    </Link>
          ...
          <div className="space-y-4">
            ...
            <div className="card">
              <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 dark:border-slate-700">
                <h2 className="text-sm font-semibold text-slate-900">Recent Notes</h2>
              ...
              <div className="p-2 space-y-1">
                {recentNotes.map((note) => (
                  <Link key={note.id} href="/notes">
                    <div className="flex items-start gap-2 px-3 py-2.5 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors group">
            ...
            <div className="card">
              <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 dark:border-slate-700">
                ...
                <Link href="/projects" className="text-xs text-primary-600 hover:text-primary-800 font-medium">
                  View all
                </Link>
              </div>
              <div className="p-2 space-y-1">
                {projects.slice(0, 3).map((project) => (
                  <Link key={project.id} href={`/projects/${project.id}`}>
                    <div className="flex items-center gap-2.5 px-3 py-2.5 rounded-lg hover:bg-slate-50 transition-colors">
                      <span
                        className="w-7 h-7 rounded-lg text-base flex items-center justify-center shrink-0"
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-slate-400">
                          {project._count?.todos} tasks · {project._count?.notes} notes
                        </p>
                      </div>
                    </div>
                  </Link>
